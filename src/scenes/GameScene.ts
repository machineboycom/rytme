...
export class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.hitArea = this.add.zone(this.btnCX, this.btnCY, this.btnW, this.btnH).setInteractive()
    this.hitArea.input!.enabled = false
    this.hitArea.on('pointerdown', () => this.onTapDown())
    this.hitArea.on('pointerup', () => this.onTapUp())

    this.tapLabel = this.add.text(this.btnCX, this.btnCY, 'TRYKK', {
    ...
    this.countdownText = this.add.text(width / 2, this.gridY + gridH / 2, '', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '72px',
      color: colors.textWhite,
    }).setOrigin(0.5).setAlpha(0)

    this.infoText = this.add.text(width / 2, height - 20, 'rytme', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '13px',
  ...
  private scheduleBeats(start: number, count: number, snareAt: ((i: number) => boolean) | null): void {
  ...
  private enterResult(): void {
    ...
    if (score === 16) {
      ...
      this.statusText.setText(`${score}/16  (${missed} tapt, ${wrong} bom)`)
    } else {
      this.statusText.setText(`${score}/16  (${missed} tapt, ${wrong} bom)`)
  ...
  private drawGrid(): void {
    ...
    for (let bar = 0; bar < 4; bar++) {
      for (let beat = 0; beat < 4; beat++) {
        ...
        if ((this.state === 'listening' || this.state === 'playing') && idx === this.lastHighlight) {
